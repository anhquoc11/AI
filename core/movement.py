...
def find_safe_orders(selected_orders, fuel, grid, start, algo):
    ...
    for k in range(len(selected_orders), 0, -1):
        for candidate in itertools.combinations(selected_orders, k):
            candidate = list(candidate)
            houses = [order["pos"] for order in candidate]
            ...
            history.append({"orders": candidate,"cost": total_cost,"safe": safe})
            if safe:
